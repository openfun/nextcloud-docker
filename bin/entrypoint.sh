...
#
#     docker run --rm -e USER_NAME=foo -e HOME='/home/foo' marsha:latest python manage.py migrate
#
...
#     HOME=/home/foo
#
#     docker run --rm --env-file env.d/production marsha:latest python manage.py migrate
#
set -e
...
if ! whoami > /dev/null 2>&1; then
  if [ -w /etc/passwd ]; then
    echo "${USER_NAME:-default}:x:$(id -u):$(id -g):${USER_NAME:-default} user:${HOME:-/home/default}:/sbin/nologin" >> /etc/passwd
  fi
fi
